<section class="panel">
    ...
    <div class="panel-body">
        <form class="form-horizontal bucket-form ajax-form" method="put" action="/galleries/{{ $record->id }}" data-target="#record-detail">
            {{ csrf_field() }}
...
<div class="col-lg-6">
    @can('update', $record)
        <div class="well">
            <h3>Add Photos</h3>
            <form action="/galleries/{{ $record->id }}/photos" method="POST" class="dropzone" id="bp-dropzone">
                {{ csrf_field() }}
            </form>
        </div>
    @endcan
